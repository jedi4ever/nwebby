...
function renderDir(srcDir, destDir, optRenderPattern) {
  let renderPattern = '**';
  if (optRenderPattern) {
    console.log("overriding render pattern", optRenderPattern)
    renderPattern= path.join(srcDir,optRenderPattern);
  } else {
...
var handleEntry = function handleEntry(filename, callback) {
  ...
  if (!path.isAbsolute(filename)) {
    fullFilePath = path.join(rssOptions.baseDir,filename);
  }
